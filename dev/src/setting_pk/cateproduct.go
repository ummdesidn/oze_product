...
func ProductCate_New(c *gin.Context) {
	...
	ctx, client, dbName := DBConn()
	defer client.Disconnect(ctx)
	coll_p := client.Database(dbName).Collection("tbl_ProductCate")
	filter := bson.M{}
	//cursor, err := coll.Find(ctx, filter)
	count, _ := coll_p.CountDocuments(ctx, filter)
	count = count + 10

	coll := client.Database(dbName).Collection("tbl_ProductCate")
	EmpNew := bson.M{"CateName": CateProduct, "Status": Status, "GenCateID": count}
